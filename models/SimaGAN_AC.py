...
class Discriminator(tf.keras.Model):
    def __init__(self, nclass, dim=64, max_filters=512, norm='in', act='lrelu', num_downsamples=5):
        super().__init__()
        ...
        self.to_logits = LinearBlock(nclass, act='none')
...
class SimaGAN_AC(tf.keras.Model):
    ...
    def train_step(self, inputs):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            # discrimination
            critic_real, logits_real = self.Disc(x)
            critic_fake, logits_fake = self.Disc(x_fake)
            ...
            d_loss, g_loss, gp = adv_loss(
                ...
                x, xr, self.Disc,
                self.gan_loss, self.gp_type, 2, None)
            ...
            # classification loss
            l_cls_g = ce(y, logits_fake)
            l_cls_d = ce(y, logits_real)

            # total loss
            l_G = g_loss + g_loss_l + self.lambda_r * l_r + l_cls_g
            l_D = d_loss + d_loss_l + 10 * gp + l_cls_d
